...
public class SubjectImpl implements SubjectDao {
	...
	public SubjectImpl() {
		operator = new TableOperator<Subject>(PsHelper.DATASOURCE, "subject", Subject.class);
		//// 针对包含自增字段的表，使用下面的构造函数
		// operator = new TableOperator("SYSDATASOURCE_DEFAULT", "subject",
		//// Subject.class,"IDENTITY_FIELD_NAME");
	}
	 ...
	 * （一般地，我们复杂查询操作使用QueryOperator比较方便） public Subject getSubject(String
	 * id)throws DBOperatorException { Subject subject = null; QueryOperator
	 * query = new QueryOperator();
	 * query.setDataSourceName("SYSDATASOURCE_DEFAULT");
	 * query.setSql("select "+operator.generateFieldsString()+" from "+operator.
	 * getTableName()); query.addParameter(new SqlParameter(SqlType.VARCHAR,
	 * id)); query.access(); RsDataSet rs = new
	 * RsDataSet(query.getSqlResultSet()); if (rs.rowCount > 0) { subject = new
	 * Subject(); subject.setSubjectNo(rs.getFieldValueAsString("subjectNo"));
